...
import { getPartnerStores, deletePartnerStoreById } from 'apiSdk/partner-stores';
import { PartnerStoreInterface } from 'interfaces/partner-store';

type ColumnType = ColumnDef<PartnerStoreInterface, unknown>;

interface PartnerStoreListPageProps {
...
export function PartnerStoreListPage(props: PartnerStoreListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PartnerStoreInterface>>(
    () => `/partner-stores?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deletePartnerStoreById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: PartnerStoreInterface) => {
    if (hasAccess('partner_store', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/partner-stores/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <>
          {hasAccess('partner_store', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/partner-stores/edit/${record.id}`} passHref legacyBehavior>
              <Button
          ...
          {hasAccess('partner_store', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="partner_store" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Partner Store
        </Text>
        {hasAccess('partner_store', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/partner-stores/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
